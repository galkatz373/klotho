...
//!
//! Integrates awake island velocities and emits `SpaceDelta`. The kernel
//! derives swept (K24) and queries `space_ix` (K23). No friction, stacking,
//! joints, or hidden integrator fields.
//!
//! `#![forbid(unsafe_code)]`.
...
use klotho_core::{BlobId, HullWitness, PoseMm, Sigil, Tick, VelFx};
...
mod tests {
    ...
    fn awake64_is_under_four_ms() {
        ...
        for i in 0..64u128 {
            let s = actor(i + 10);
            let mut w = k.world_mut();
            w.insert_locus(s, LocusKind::Actor).unwrap();
