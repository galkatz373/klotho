...
| Author | Gal Katz |
| Date | 2026-09-14 |
| Status | Current (rev 8 — PHYS-A01–A04 landed) |
| Last verified | 2026-09-14 through PHYS-A04 |
...
| **K61** | **Canonical geometry is shared, typed, and narrow.** PHYS-A03 added `klotho-geom`, depending only on `klotho-core`; both `klotho-phys` and `klotho-commit` depend on it, but commit still never depends on phys. The frozen cooked shape vocabulary is oriented box, sphere, Y-axis capsule transformed by the locus pose, convex hull, compound hull, and static triangle mesh or heightfield. PHYS-A04 implements the first five with bounded convex vertices and compound children; terrain kinds still fail closed. Dynamic bodies may use the first five; triangle mesh/heightfield is static occupancy only. Shape ids and quantized witness identity remain in `klotho-core`. | Commit must independently verify contacts without importing a solver or accepting an opaque manifold. Static terrain must not join dynamic islands. |
...
- **Depends on:** PHYS-A03
- **Changes:** Adds bounded convex and compound geometry, stable four-point contact patches, conservative convex thin-wall casts, and Canon-bound body mode, shape, mass, centre of mass, diagonal inertia, friction, and restitution. Phys integrates angular requests and contact response on the pinned scalar lane; commit independently checks Canon shape kinds and quantized witnesses. Terrain kinds remain fail-closed for PHYS-A05.

#### PHYS-A05–A12 — terrain and constraints through release acceptance — **planned**

- **Next:** PHYS-A05 lands static mesh/heightfield queries, slopes, fixed/hinge/spring constraints, break witnesses, and sleeping/waking gates.
- **Then:** character drive, semantic tracks, motion-contact admission, vehicles, breakable structures, Anvil diagnostics, and release acceptance follow in the order frozen by the plan.
