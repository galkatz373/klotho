...
| Author | Gal Katz |
| Date | 2026-09-15 |
| Status | Current (rev 12 — PHYS-A01–A12 gates landed; Phys throughput open) |
| Last verified | 2026-09-16 through PHYS-A12 acceptance gates |
...
This is the current architecture for a studio that wants Klotho's programming model **and** a contemporary first-party quality bar. The preserved rev 5 baseline remains useful history, but this document is the law where the two differ. **Landed on `main` (verified through PHYS-A12 acceptance gates):** AAA-01, AAA-02, AAA-03, AAA-04, AAA-05, AAA-08.1, AAA-06, AAA-08, AAA-07, AAA-09, AAA-10, AAA-11, AAA-11b, AAA-12, AAA-13, AAA-14, AAA-15, AAA-16, AAA-17, AAA-18, AAA-19, AAA-20, AAA-21, AAA-21b, AAA-22, AAA-23, AAA-24, AAA-25, AAA-26, AAA-27, PHYS-A01, PHYS-A02, PHYS-A03, PHYS-A04, PHYS-A05, PHYS-A06, PHYS-A07, PHYS-A08, PHYS-A09, PHYS-A10, PHYS-A11, and PHYS-A12 (plus rev-4 K58 caps). **The planned AAA sequence is complete; Phys throughput remains an open release gate.**
...
CI: same `.warp` + Intent file ⇒ same Trace prefix hash on linux/mac/windows for **kernel + Rite + kinematic Motion/Space**. **`PhysIsland` goldens use a pinned Linux image** (K44): three-OS hash is not claimed for vehicles/stacking. Overlay, PoseDelta, rewind ring, Manifest, GPU, ragdoll are never in that hash.
...
| **K55** | **Exclusive spatial owner is admit-time.** After Player (and Rel writes this tick), exactly one of Phys, Space, Motion may **commit** a pose for a Sigil. Default: unattached `Actor` → Motion; `Driveable` / rigid relics → Phys; leftover kinematic → Space; `PilotedBy`/`AttachedTo` children → Phys. Jobs may **over-produce** (MotionDelta for a driver who possesses this tick); the extra nacks `Conflict`. `write_cells(PhysIsland)` **includes every currently attached child** of every body. Motion/Space skip attached actors when the **pre-admit** view already shows the Rel; same-tick possess is the nack path. Drift golden: possess at T, no Motion root on the driver at T. **Do not** tag owner in Partition and skip at propose time — ProposeJobs run *before* Player admit, so same-tick `PilotedBy` is invisible. A later RFC may admit Player **before** Partition; that is a phase-order change, not a free K55 fix. Extra MotionDelta for one driver is not the 10 ms propose budget. `admit_key` is a total order; Conflict is deterministic. | ProposeJobs run *before* Player admit. Owner-at-propose-time is a race with `PilotedBy`. |
| **K56** | **Attach is a kernel fact.** `Rel::PilotedBy` (Q12 closed). Optional `Rel::AttachedTo`. Era 1 compose is **yaw-only, not SO(3)**: `child.xz = parent.xz + rot_yaw(parent.yaw, attach_local.xz)`; `child.y = parent.y + attach_local.y`; **copy** parent yaw/pitch/roll onto the child (seat, not turret). General 6DOF welds are a later RFC. Motion does not integrate attached actors. `support` is written only from an admitted physical island (or Motion swept for unattached). **Ban** Motion→phys calls. Drift v1: one vehicle + driver, flat AABB floor. | Millidegree Euler compose is gimbal-ambiguous. A seat copies parent attitude. |
...
| --- | --- | --- | --- |
| `klotho-geom` | Pure canonical shape queries and witness verification | No | Depends only on core; shared by phys and commit (PHYS-A03) |
| `klotho-phys` | Rigid + kinematic island proposer | **Yes** (SIMD of scalar XPBD; **no FFI in Era 1**) | Emits one `PhysIsland` per solved island. No `&mut World` |
...
**Hitch vs K21:** Residency stays **one** transaction (`conflict_set` = every `PackedIx` in the PlaceSnap). Partial Place is still a kernel bug. Do **not** chunk a visible apply. Era 1 Places are sized so apply fits 2 ms (**author ≤ 10k rows** until the microbench says otherwise; process per-Place cap 100k stays as a bomb). Optional: decode off-thread into an unpublished spec, then one admit + one publish. Drift is two Places on a strip — not a 100k-row Place.
...
`OpaqueClosed` sleepers **remain in `space_ix`** (rev 5 K23 — an idle locked door still blocks). Dormant is not "deleted." Dormant / idle `OpaqueClosed` / Place floors are **not** partition seeds and **not** flood-fill members; they still block. Far agents still exist; they are omitted from `PoseDelta` except a cheap keep-alive, and they do not write 2 Hz `IslandSnap` unless interacting.
...
**Vehicles (K56):** `Affordance Driveable`. PHYS-A09 binds an optional four-wheel ray-cast rig on `BodyPhysics::vehicle` (hubs, rest, spring/damper, radius, steer, tire scales). Phys consumes throttle/brake/steer from chassis or attached-driver `PhysRequest` and admits the chassis pose only. Unbound Driveable relics keep the legacy one-shot PHYS_REQ Δv fold so Drift goldens stay put. Traction rays hit canonical occupancy, including heightfield ramps. `Verb::Steer` (AAA-08.1). Driver: `Rel::PilotedBy`. Era 1 attach compose (**not** SO(3) Euler multiply):
...
**Solver law (K44 / Q9 closed):** Era 1 ships **in-house or vendored scalar XPBD**, **no FFI**, SIMD optional and identical in CI and every dedicated server. Rapier/Jolt rejected as hashed truth. Client phys = Overlay only. No warm-start unless hashed. Ember/Drift phys goldens = pinned Linux, not three-OS Trace. Phys crate: no fast-math, fp-contract off, no OS-varying FMA. Residual gate: p99 ≤ 1 mm, any sample ≤ 4 mm, or AAA-08 fails.
...
Island identity is `(epoch, tick, partition_id, sorted member Sigils)`. Admission requires the proposal epoch and tick to equal the live step, `partition_id != NO_ISLAND`, and `members` to equal the current K58 partition exactly. `bodies` must include every unattached Relic member; explicitly Canon-driven Actors are required from PHYS-A06 onward; unbound Actors remain Motion-owned and attached rows derive from their parent. Every body and constraint must name its current Canon shape/constraint binding. Body, contact, constraint, or attached-child duplication; missing members or required bodies; stale bindings; out-of-order payloads; and cap overflow reject the entire island before speculative apply.
...
These four jobs **must not** share one 10 Hz `IslandSnap`. Overlay interpolating PoseDelta while Trace keeps 2 Hz snaps is **not** drift.
...
`klotho-interest` inputs: Observer poses, Canon radii per `LocusKind` / Affordance, Place AABBs. Outputs: Full/Far/Dormant bitsets + residency **commands** (runtime wraps as proposals). Deterministic. Deps: world+core only (`PhysRequest` and hulls already live on `WorldView` / `klotho-core` — do **not** add `interest → commit`). Island-wake **skips** `NO_ISLAND` so unassigned scenery does not become Full because it shares a default id. Interest still runs *before* this-tick Partition, so mate promotion uses last-tick island ids (not a crate-graph leak).
...
AAA-15 is a **production editor program** (viewport, outliner, sequencer, cook, retarget, lighting, profiler), not one PR of egui glue. `forbidden-imports.sh` is necessary and insufficient: `klotho-manifest::tables` stays `pub(crate)`; AAA-11/15 add feature gates so presenter crates cannot leak tables into `klotho-editor` gameplay paths. Do not start this until Era 1 goldens exist.
...
| `klotho-jobs` | steal, cache-pad atomics | Run admit; unordered reduction. **Miri + Loom** on the deque before further steal changes |
| `klotho-stream` | mmap, uncached IO | Insert loci except via Residency proposal. **Miri** on mmap header-validate |
| **all else** | **forbidden** | — |
...
impl Budget {
    // AAA-02 landed: pred_ops / rite_steps are u32; rewind_ticks exists.
...
pub const MAX_ISLANDS: u16 = u16::MAX;      // 65_535 groups; ids 0..=65534
pub const NO_ISLAND: u16 = u16::MAX;        // not a member; Interest skips
pub const MAX_ISLAND_SIZE: u16 = 256;       // omit whole group, never split
```

`RejectReason` adds `Residency`, `EpochMismatch`, `NotInterested` (AAA-06/18), **`TooManyIslands`**, **`IslandTooLarge`** (landed with K58 caps), and keeps `StaleEpoch` for rewind-too-old. Do not add `Predicted`.
...
pub enum Proposal {
    ...
    PhysIsland {
        ...
        tick: Tick,
        island: u16,
        members: Vec<Sigil>, // exact sorted K58 partition
...
pub struct Partition {
    ...
    pub omitted_too_large: u32, // IslandTooLarge, whole groups
    pub omitted_too_many: u32,  // TooManyIslands
}
pub fn partition_islands(view: &WorldView) -> Partition;
```
...
| --- | --- | --- |
| `SourceKind` (`pred.rs`) | Player, Mind, Space, Motion, Infer | **`Phys`**, optional `Residency` if a Law uses `SourceIs` on loads. Append. `SourceIs(Phys)` is how `never_clip_closed` sees each body in a `PhysIsland`. |
...
- `klotho.phys.awake`, `klotho.phys.rejected_budget`, `klotho.phys.quant_residual_mm` (**gate:** p99 ≤ 1 mm, max ≤ 4 mm on Ember stacking CI)
- `klotho.partition.too_large`, `klotho.partition.too_many`
...
| Risk | Sev | Mitigation |
| --- | --- | --- |
| Partition mega-island (floor hull / touching walls) | **Critical** | K58: occupancy ≠ island; Place/idle OpaqueClosed not members; `MAX_ISLAND_SIZE` omit-not-split; Drift floor is not a body |
...
| Phys not hash-stable | **High** | Scalar-only path; no warm-start; pinned-Linux goldens; **no** three-OS `PhysIsland` hash |
...
#### AAA-01 — Trace pose discipline (fix the tape) — **landed**
...
#### AAA-02 — Integer 6DOF pose, `Vel3`, budget profiles — **landed**
...
#### AAA-05 — `klotho-interest` + SimLod — **landed**
...
#### AAA-08.1 — Pred-lang / ISA / Verb / Rel / TraceBody — **landed**
...
#### AAA-24 — Cook farm / large warp — **landed**
...
#### AAA-25 — Canon epoch packs (live ops) — **landed**
...
#### AAA-26 — Console platform spike — **landed**
...
#### PHYS-A02 — Atomic `PhysIsland` admission — **landed**

- **Files:** `klotho-commit`, `klotho-phys`, `klotho-world`, Drift and Netlock migration tests
- **Depends on:** PHYS-A01
- **Changes:** Replaces `PhysDelta` with one bounded island proposal carrying exact K58 membership and sorted body/contact/constraint lanes. Validates epoch, tick, membership, bindings, ordering, uniqueness, and caps before one speculative apply. The complete body/attached-child write set conflicts atomically; Laws run in `(LawId, body Sigil)` order over the complete proposed state. One invalid body rolls every body back byte-for-byte. Constraint/break payloads were fail-closed here and are applied by PHYS-A05.

#### PHYS-A03 — Shared geometry and oriented primitives — **landed**

- **Files:** `klotho-geom`, `klotho-core`, `klotho-commit`, `klotho-phys`
- **Depends on:** PHYS-A02
- **Changes:** Adds `klotho-geom` (core-only) with oriented box, sphere, and capsule queries, ray/shape casts, and quantized witness reproduction. Commit verifies PhysIsland motion and gameplay contacts against that geometry; Space/Motion keep translation-AABB swept so Hearth/Ash hashes stay put. This was the primitive foundation extended by PHYS-A04 and PHYS-A05.

#### PHYS-A04 — Convex/compound contact and CCD — **landed**

- **Files:** `klotho-core`, `klotho-canon`, `klotho-world`, `klotho-geom`, `klotho-phys`, `klotho-commit`
- **Depends on:** PHYS-A03
- **Changes:** Adds bounded convex and compound geometry, stable four-point contact patches, conservative convex thin-wall casts, and Canon-bound body mode, shape, mass, centre of mass, diagonal inertia, friction, and restitution. Phys integrates angular requests and contact response on the pinned scalar lane; commit independently checks Canon shape kinds and quantized witnesses. Terrain kinds were fail-closed here and land in PHYS-A05.

#### PHYS-A05 — Static terrain and constraints — **landed**

- **Files:** `klotho-core`, `klotho-canon`, `klotho-world`, `klotho-geom`, `klotho-phys`, `klotho-commit`
- **Depends on:** PHYS-A04
- **Changes:** Adds static triangle-mesh and heightfield occupancy queries, slope contact with friction, Canon-bound fixed/hinge/slider/spring constraints, kernel-validated break witnesses, and the 120-tick sleep/wake gate. Place/static occupancy still never joins a dynamic island; intact joints do union their endpoints. Constraint-state rows persist on Projection (snapshot v2).

#### PHYS-A06 — Character drive constraints — **landed**

- **Files:** `klotho-core`, `klotho-ir`, `klotho-canon`, `klotho-world`, `klotho-geom`, `klotho-motion`, `klotho-phys`, `klotho-commit`; seed consumers
- **Depends on:** PHYS-A05
- **Changes:** Authored `Physics` seed facts bind capsule traversal policy and up to eight quantized semantic root samples in Canon; cook hashes and warp round-trips include the binding. Motion exposes a pure drive and emits no competing pose for driven actors. Phys resolves capsule sweep, grounding, steps, slope limits, kinematic-platform translation/rotation and dynamic-crate interaction in one island. Commit requires the driven actor body, reproduces the static traversal path and independently validates final penetration. Bounded tests cover 200/250 mm risers, 30°/50° ramps, thin walls, falling, moving platforms, atomic rejection, save continuation, and one/eight-worker Trace equality. Unbound actors keep legacy Motion; contact-track cook and melee-contact coupling remain A07/A08. The 512-obstacle and 1000 mm query bounds fail closed with actor diagnostics.

#### PHYS-A07 — Semantic contact-track cook — **landed**

- **Files:** `klotho-core`, `klotho-ir`, `klotho-canon`, `klotho-prove`, `klotho-compile`, `klotho-dcc`, `klotho-editor`; configuration-only seed consumers
- **Depends on:** PHYS-A06
- **Changes:** Canon-bound, quantized root/socket/swept-capsule tracks bind skeleton, instrument, action and an exact labeled channel-bearing Rite WAIT. Caps are 65 boundaries, eight sockets/channels, sixteen foot intervals and 128 KiB encoded payload. Kind-11 CAS artifacts validate canonical integer encoding and reconstruct through warp. Compatibility certification freezes roots/timing/rig and permits at most 5 mm Euclidean socket/endpoint retarget error; contact-bound animated DCC imports require approved semantic sidecars. Cosmetic bytes stay outside the signature and Canon identity. An explicit track replacement epoch pack hashes semantic samples and advances the epoch. Distaff offers disposable read-only root/socket/sweep/plant/WAIT previews. This PR emits no gameplay hit; capsule contact admission follows in A08.

#### PHYS-A08 — Motion-contact admission — **landed**

- **Files:** `klotho-core`, `klotho-trace`, `klotho-world`, `klotho-geom`, `klotho-motion`, `klotho-phys`, `klotho-commit`, `klotho-stream`, and the contact goldens
- **Depends on:** PHYS-A07
- **Changes:** Coupled physical islands now carry bounded, Canon-exact semantic contact claims. The kernel reproduces an integer narrow-phase sample against the resolved root and target hull, checks active WAIT timing, original player Agency, compatibility identity, target affordance and per-action ledger, and admits contact consequences in the same speculative transaction. Projection snapshot v3 preserves the action instance, absolute WAIT clock, authorization channel and one-target ledger. Trace records semantic actor/target/instrument evidence without presenter poses. Runtime jobs stamp the next authoritative tick; stream rows refuse live action metadata. Pure geometry, save/replay, blocked-root, Law rollback and one/eight-worker goldens are landed. The initial policy is one target/contact per action with bounded query work; vehicles, destruction, Anvil capture and release acceptance remain PHYS-A09–A12.

#### PHYS-A09 — Vehicle rigs — **landed**

- **Files:** `klotho-core`, `klotho-ir`, `klotho-canon`, `klotho-world`, `klotho-geom`, `klotho-phys`, `klotho-compile`; seed consumers
- **Depends on:** PHYS-A08
- **Changes:** Authored `Physics` seed facts bind a two-to-four-wheel ray-cast rig (hubs, rest, spring/damper, radius, steer envelope, tire scales) on the chassis Relic. Cook hashes and warp round-trips include the binding. Phys consumes throttle, brake and steer from chassis or attached-driver `PhysRequest` and proposes only the chassis island pose. Wheel rays use shared integer geometry against occupancy, including oriented boxes and heightfield ramps; surface friction comes from the occupancy material. Tire caches stay disposable. Unbound Driveable relics keep the legacy PHYS_REQ Δv fold, so original Drift goldens are unchanged. Bounded tests cover accelerate/coast/brake/reverse, steer, excessive-speed sideslip, high/low traction ordering, slope settle, driver attach/detach, save continuation, query overflow, presented wheel rate, and one/eight-worker Trace equality. Anvil capture landed in PHYS-A11; release acceptance remains PHYS-A12.

#### PHYS-A10 — Breakable structures — **landed**

- **Files:** `klotho-core`, `klotho-trace`, `klotho-world`, `klotho-commit`, `klotho-phys`, `klotho-vfx`
- **Depends on:** PHYS-A09
- **Changes:** An admitted constraint-break witness RelDels `PartOf` between the joint endpoints, records one `ConstraintBroken` evidence event, and spawns Canon `fragments` (0..=64) as Fragment Relics with hulls in the same speculative island. Global live-fragment count is 128; a tighter Cap Law still rejects and rolls the island back byte-identical. Cosmetic chips are Manifest TTL `OneShotMesh` with no Sigil and never occupy Projection. Below-threshold impulses hold the joint and relation. Save/load restores broken state, relations, and fragment hulls/poses. One- and eight-worker execution match. Anvil capture landed in PHYS-A11; release acceptance remains PHYS-A12.

#### PHYS-A11 — Anvil and diagnostics — **landed**

- **Files:** `anvil-slice`, `klotho-phys`, `klotho-debug`, Distaff
- **Changes:** One bounded Anvil scene combines the rotated five-crate stack, 200/250 mm stair and 30°/50° slope geometry, hinge and breakable constraints, platform rider, dynamic crate push, and player-authorized semantic sword crossing on one `CommitKernel`. A 30-tick golden observes the semantic hit, quantity change, one break, push, and platform motion. An invalid member rejects an island byte-identically. Published-snapshot island captures retain Canon identity, epoch, next tick and exact proposal payload; replay recomputes the pure proposer output. Read-only overlays expose canonical hulls, proposed and captured poses, root desire and resolution, step/slope policy, WAIT sweeps, support/sleep, and constraint impulse/threshold. Distaff requests captures without admitting them. Kernel reject reasons have literal diagnostic explanations. Proposer stages emit disposable broad, character, vehicle, narrow, constraint and encode timing samples; summaries report p50/p95/p99/max and workload maxima. Capture remains in memory; release acceptance is PHYS-A12.

#### PHYS-A12 — Release acceptance gates — **landed**

- **Files:** `anvil-slice`, `klotho-world`, `klotho-save`, `klotho-ui`, `klotho-release`, pinned Phys CI.
- **Changes:** The pinned Linux x86-64/FMA-off job freezes the 120-tick Anvil Trace prefix at `9075e1b7db1a8374889ed3965aa4c65ec39d655bf9ffd3d8e77ba192b0763564`. The three-OS workspace matrix runs behavioral envelopes. Anvil resumes combined action, structure and movement from exact pause saves at ticks 3, 16 and 31, then compares Trace and Projection through tick 60. Thirty consecutive captures replay exactly. A 2,000-body independent-island fixture checks admission capacity, and a 120-tick run checks bounded behavior and repeatability. Save v2 records OS/CPU origin for movable-hull snapshots and refuses cross-platform load; v1 unlabelled saves are refused. The crate firewall remains gated.
- **Remaining release gate:** This evidence does not certify the AAA adventure timing budget. On a local optimized ARM Mac build, a synthetic 2,000-island serial tick took about 1.3 s. The first-title Phys profile must still meet the 10 ms propose, 8 ms admit and 21 ms critical-path limits on a representative pinned runtime workload before release.

---

*End of successor HLD (rev 11). Klotho's product is still Canon / Intent / Trace / Projection. Pose has three channels with an explicit rebuild matrix. PackedIx is u32 (not pred Slot) and already landed. Partition produces **phys-body** islands before jobs; occupancy is not an island; oversize groups fail closed, they are not split. K55 remains admit-time over-produce-then-nack for legacy paths; K63 now gives Canon-driven characters one island owner. Physics admission is island-atomic under K59, with shared canonical geometry and Rite-gated semantic contact under K61–K65. Phys is scalar XPBD with pinned FMA-off + residual gate. GI is probes+SSGI. Infer is an OS-process sidecar. Automatic and pause saves are exact fresh snapshots. AAA is capacity, proposers, and presentation — not a component bag, not a second physics world, not Predicted-on-Trace, and not a year-1 Unreal clone. If a PR puts `&mut World` in infer, a scene file as truth, f32 in Projection, parallel admit, Jolt as hashed truth, a CombatManager crate to ship Ember, or a floor hull as one giant island, it is a bug, not a feature.*
